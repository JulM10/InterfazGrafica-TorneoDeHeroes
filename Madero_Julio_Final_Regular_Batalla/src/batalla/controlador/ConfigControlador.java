...
public class ConfigControlador {
    ...
    private void cargarPersonajesDesdeDB() {
        ...
        for (Personaje p : existentes) {
            ...
            vista.agregarFilaTabla(
                    ...
                    p.getVida(), p.getFuerza(), p.getDefensa(), p.getVictorias());
        }
    }
    ...
    private void agregarPersonaje() {
        ...
        vista.agregarFilaTabla(nombre, apodo,
                ...
                nuevo.getVida(), nuevo.getFuerza(), nuevo.getDefensa(), nuevo.getVictorias());
        vista.limpiarCampos();
    }
    ...
    private void eliminarPersonaje() {
        ...
        Personaje p = personajes.get(fila);

        int confirmar = JOptionPane.showConfirmDialog(null,
                "¿Seguro que queres eliminar a " + p.getNombre() + " (" + p.getApodo() + ")?\n" +
        ...
        if (p.getId() != 0) {
            try {
                PersonajeDAO dao = new PersonajeDAO();
                dao.eliminar(p);
                vista.mostrarInfo("Personaje " + p.getNombre() + " eliminado correctamente.");
            } catch (Exception e) {
                vista.mostrarError("Error al eliminar de la BD: " + e.getMessage());
                e.printStackTrace();
                return;
            }
        }
